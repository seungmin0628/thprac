---
name: thprac-debug
description: Build and debug this thprac repository with the Windows-native Codex agent, Visual Studio/MSBuild, named thprac/Touhou sessions, GUI verification, and collected diagnostics. Use for thprac implementation, build failures, launcher or injection issues, and game-specific runtime verification in this repository.
---

# thprac Windows-native Debug Loop

Work from the repository root opened by its native Windows path. Use the Windows-native Codex agent and PowerShell for the supplied scripts; do not substitute a WSL compiler for this Win32 Visual Studio project. If the current task uses a `/mnt/...` or `\\wsl$` working directory, use it only for non-GUI work or reopen the repository as a Windows-native task before claiming visual verification.

## Loop

1. Read `AGENTS.md`, inspect the affected code, and define the smallest observable verification for the change.
2. Build Debug from Windows PowerShell:
...
3. For repeatable local paths, copy `.codex/thprac-debug/config.example.json` to the ignored `config.json`, edit it, and validate aliases with `debug-session.ps1 games`. Start a named session using the affected game alias:
   ...
   ```powershell
   powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\scripts\debug-session.ps1 start -Name th06-smoke -Game th06
   ```
...
4. Confirm the session with `debug-session.ps1 status -Name <name>`. For an external-attach session, require the game entry to report `integration=True`; a running wrapper alone is not success. With the Windows desktop unlocked and the target window visible, use Computer Use to inspect and operate the thprac/Touhou GUI; never use it to automate a terminal. Record the game, version, screen or menu path, and observed result. If Computer Use is unavailable, report process-level verification separately and do not claim GUI behavior was verified.
5. Collect evidence before stopping a failed or crashed session:

   ```powershell
   powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\scripts\collect-debug.ps1 -Name th06-smoke
   ```
...
- Keep the repository on a Windows-native drive for the primary workflow. WSL2 may call the PowerShell scripts as a fallback, but it is not the GUI-verification environment.
