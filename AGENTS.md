...
- For managed game keyboard verification, the user explicitly authorizes `scripts\game-input.ps1` as an exception to Computer Use-only input. Use Computer Use for observation and mouse actions; use this session-bound scan-code helper for timed game keys/chords. Read `scripts/game-input.md`. Provision and validate each game environment once, then start feature verification directly using its recorded setup; do not repeat a separate input-readiness preflight on every development task. Recheck environment readiness only after relevant installation/input/launch changes or an actual failure. Keep automatic session identity/focus guards on every input. Do not stop at failed instantaneous key presses or mistake automatic demo playback for controlled gameplay.
