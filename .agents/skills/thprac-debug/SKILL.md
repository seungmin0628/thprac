...
8. When GUI interaction is necessary, load the available Windows Computer Use skill, select the exact returned session window, observe, act, and reobserve. Build, start/stop, and collection remain PowerShell operations.
   For game keyboard input, follow the user-authorized exception in AGENTS.md: read scripts/game-input.md and use scripts/game-input.ps1 with the exact observed session ID. For an already validated game setup, begin the requested feature scenario directly without a separate readiness preflight. Validate controls during initial environment provisioning, or after relevant environment changes/failure. Handle title/demo timeouts with bounded known sequences and pause gameplay during reasoning. Input delivery is not proof of behavior; verify the screen after each meaningful sequence.
